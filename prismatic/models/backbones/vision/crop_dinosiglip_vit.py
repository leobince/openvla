...
class CropDinoSigLIPViTBackbone(VisionBackbone):
    ...
    def forward(self, pixel_values: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        dino_patches = rearrange(dino_patches, '(batch crop) token dim -> batch (crop token) dim', crop=5) 
        siglip_patches = rearrange(siglip_patches, '(batch crop) token dim -> batch (crop token) dim', crop=5) 
