...
from ..util.utils import has_files_with_suffix
...
def load_vla(
    ...
    if os.path.isfile(model_id_or_path) or os.path.isdir(model_id_or_path):
        # 首先检查模型的文件结构是单一权重文件还是多个权重文件
        checkpoint_pt = has_files_with_suffix(model_id_or_path, ".safetensors")
        if checkpoint_pt != False:
            overwatch.info(f"Loading from local checkpoint path `{model_id_or_path = }`")
            config_json, dataset_statistics_json = model_id_or_path / "config.json", model_id_or_path / "dataset_statistics.json"
